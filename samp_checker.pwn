...
{
    new name[MAX_PLAYERS], string[256], ip[16];
    ...
    {
        format(string, sizeof(string), "[LOCALHOST] %s(%d) has joined the server.", name, playerid);
    ...
    {    
        if (data[0] == 'Y')
        {
            format(string, sizeof(string), "[PROXY DETECTED] %s(%d) has been kicked from the server.", name, playerid);
        ...
        {
            format(string, sizeof(string), "[PROXY NOT DETECTED] %s(%d) thank you for joining!", name, playerid);
            SendClientMessageToAll(0x09F7DFC8, string);
        }
        else if (data[0] == 'X')
        {
...
public OnPlayerSpawn(playerid)
{
    ...
    {
        SendClientMessage(playerid, -1, "You are using Android Platform!");
    } 
    else 
    {
        SendClientMessage(playerid, -1, "You are using PC Platform!");
    }
    ...
    {
        SendClientMessage(playerid, -1, "Your Client is Version 0.3.DL-R1");
    }
    else if (strcmp(version, "0.3.7-R1", true) == 0)
    {
        SendClientMessage(playerid, -1, "Your Client is Version 0.3.7-R1");
    }
    else if (strcmp(version, "0.3.7-R2", true) == 0)
    {
        SendClientMessage(playerid, -1, "Your Client is Version 0.3.7-R2");
    }
    else if (strcmp(version, "0.3.7-R3", true) == 0)
    {
        SendClientMessage(playerid, -1, "Your Client is Version 0.3.7-R3");
    }
    else if (strcmp(version, "0.3.7-R4", true) == 0)
    {
        SendClientMessage(playerid, -1, "Your Client is Version 0.3.7-R4");
    }
    else if (strcmp(version, "0.3.7-R5", true) == 0)
    {
        SendClientMessage(playerid, -1, "Your Client is Version 0.3.7-R5");
    }
    return 1;
}
